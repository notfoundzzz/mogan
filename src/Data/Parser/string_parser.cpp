...
string_parser_rep::do_parse (string s, int& pos) {
  ...
  if (m_start_size <= 0) {
    if (DEBUG_PARSER)
      debug_packrat << "m_start is empty unexpectedly with " << pos << ":" << s
                    << LF;
    return;
  }
...
bool
string_parser_rep::unfinished () {
  return !m_finished;
}

void
string_parser_rep::do_finish () {
  m_start_size= 0;
  m_finished  = true;
}

void
string_parser_rep::set_pairs (hashmap<string, string> p_pairs) {
  m_pairs= p_pairs;
}
...
void
string_parser_rep::set_escaped_char_parser (
    escaped_char_parser_rep p_esc_parser) {
  m_esc_parser  = p_esc_parser;
  use_esc_parser= true;
}

bool
string_parser_rep::parse_escaped (string s, int& pos) {
  m_escaped= false;
...
void
string_parser_rep::skip_escaped (bool skip) {
  m_skip_escaped= skip;
}
