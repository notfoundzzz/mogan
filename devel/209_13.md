...
## 2026/02/04 代码悬浮菜单高度与 package 自动加载修复

### What

- 修复 KDE / Plasma 下代码悬浮菜单按钮高度不足，导致文本和图标被裁剪的问题。
- 在 `code-menu` 的 Inline / Block / Listing 创建入口统一接入 `code-popup-make`。
- 让 `code-popup-make` 在创建 code tag 前自动确保对应 package 已加载，避免“函数已定义但未生效”。

### How

- Qt 侧修改 `src/Plugins/Qt/QTMCodePopup.cpp` 的 `QTMCodePopup::autoSize()`：
  - 移除基于 DPI / `magf` 的外框放大逻辑；
  - 仅使用按钮 `sizeHint` + 布局边距计算弹窗尺寸；
  - 保留最小兜底尺寸，避免异常情况下尺寸过小。
- Scheme 侧修改 `TeXmacs/progs/prog/prog-menu.scm`：
  - 增加 `(prog prog-edit)` 依赖；
  - 将代码语言项从 `make` / `make*` 改为 `code-popup-make`，统一走自动 package 确保逻辑。
